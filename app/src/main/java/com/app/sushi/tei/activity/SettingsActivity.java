...
public class SettingsActivity extends AppCompatActivity {
    ...
    ToggleButton togglenotification, toggleemail;
    String notification_status = "0", email_status = "0";
    Context mContext;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toggleemail = (ToggleButton) findViewById(R.id.toggleemail);
        ...
        if (Utility.readFromSharedPreference(mContext, GlobalValues.NOTIFICATION_STATUS).equalsIgnoreCase("1")) {
        ...
        if (Utility.readFromSharedPreference(mContext, GlobalValues.EMAIL_STATUS).equalsIgnoreCase("1")) {
            toggleemail.setChecked(true);
            email_status = "1";
        } else {
        ...
        togglenotification.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (togglenotification.isChecked()) {
                    ...
                    Utility.writeToSharedPreference(mContext, GlobalValues.NOTIFICATION_STATUS, notification_status);
                    updateNotification(notification_status, email_status);
                } else {
                    togglenotification.setChecked(false);
                    notification_status = "0";
        ...
        toggleemail.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (toggleemail.isChecked()) {
                    toggleemail.setChecked(true);
                    email_status = "1";

                    Utility.writeToSharedPreference(mContext, GlobalValues.EMAIL_STATUS, email_status);
                    updateNotification(notification_status, email_status);
                } else {
                    toggleemail.setChecked(false);
                    email_status = "0";
...
//                            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
    ...
    private void updateNotification(String notification_status, String email_status) {
        if (Utility.networkCheck(mContext)) {

            String url = GlobalUrl.UPDATE_NOTIFICATION_URL;
            ...
            params.put("notify_push", notification_status);
            params.put("notify_email", email_status);

            new UpdateNotificationStatus(params).execute(url);
        } else {
            Toast.makeText(mContext, "Please check your internet connection.", Toast.LENGTH_SHORT).show();
        }
    }

    private class UpdateNotificationStatus extends AsyncTask<String, String, String> {

        private ProgressDialog progressDialog;
        private Map<String, String> updateNotiParams;

        UpdateNotificationStatus(Map<String, String> updateNotiParams) {
            this.updateNotiParams = updateNotiParams;
        }
        ...
        protected String doInBackground(String... params) {
            ...
            String response = WebserviceAssessor.postRequest(mContext, params[0], updateNotiParams);
...
//                                }
                            ...
                            NavigationAdapter navigationAdapter = new NavigationAdapter(SettingsActivity.this, navigateMenuArrayList);
            ...
            if (response != null) {
                ...
                try {
                    ...
                    if (status.equals("ok")) {    //Success
                        ...
                        String notify_Email = resultJSON.getString("customer_email_notify");
                        String notify_Push = resultJSON.getString("customer_push_notify");
                        Utility.writeToSharedPreference(mContext, GlobalValues.NOTIFICATION_STATUS, notify_Push);
                        Utility.writeToSharedPreference(mContext, GlobalValues.EMAIL_STATUS, notify_Email);
