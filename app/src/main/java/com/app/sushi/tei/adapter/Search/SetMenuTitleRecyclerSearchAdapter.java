...
public class SetMenuTitleRecyclerSearchAdapter extends RecyclerView.Adapter<SetMenuTitleRecyclerSearchAdapter.TitleHolder> {

    private Context mContext;
    private List<SetMenuTitle> setMenuTitleList;
        private SetMenuChildRecyclerSearchAdapter setMenuChildRecyclerAdapter;
    private int minmaxSelect = 0;
    private String Status="";


    public SetMenuTitleRecyclerSearchAdapter(Context mContext, List<SetMenuTitle> setMenuTitleList, int minmaxSelect, String Status) {
    ...
    public void onBindViewHolder(final TitleHolder holder, int position) {
        ...
        if (setMenuTitleList.get(position).getSetMenuModifierList() != null) {
            if (setMenuTitleList.get(position).getSetMenuModifierList().size() > 0)
            {
                ...
                setMenuChildRecyclerAdapter = new SetMenuChildRecyclerSearchAdapter(mContext,
